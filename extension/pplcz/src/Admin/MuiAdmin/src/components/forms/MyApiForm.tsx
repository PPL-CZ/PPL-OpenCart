...
const MyApi = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return fetch(url).then(x => x.json() as Promise<MyApiModel>);
    },
  });
      ...
      await fetch(url, {
        ...
        .then(async x => {
          ...
          if (x.status === 400) {
            ...
            if (message.data.errors[""]) setError("client_secret", { message: message.data.errors[""] });
            else {
