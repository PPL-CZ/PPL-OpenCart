...
class CPLBatchCreateShipmentsDenormalizer implements DenormalizerInterface
{
    ...
    public function denormalize($data, string $type, ?string $format = null, array $context = [])
    {
        ...
        $createShipment->setShipments(array_map(function ($item) use ($normalizer, $shipmentLoader) {
            if (!($item instanceof ShipmentData))
                $item = $shipmentLoader->load(Shipment::class, $item);
            ...
            try {
                return $normalizer->denormalize($item, EpsApiMyApi2WebModelsShipmentBatchShipmentModel::class);
            }
